...
<body class="u_scroll">
	<div class="up-container-fluid">
		<form
			...
			<div class="up-form-group">
				<label for="">主题 </label> <select name="opt" id="selid"
					class="up-form-control">
					<option value="">请选择主题</option>
					<c:forEach items="${selist}" var="item">
						<option value="${item.ID}">${item.FD_ZTMC}</option>
					</c:forEach>
				</select>
			</div>
			<div class="up-form-group">
				<label for="">信息分类 </label> <input type="text"
			...
			<div class="up-form-group">
				<button type="button"
					class="up-btn up-btn-sm up-btn-default up-btn-block" id="xxfladd">
					<i class="icon-u-add-round"></i>新增
				</button>
				<%--                     <a href="${basePath}/xxflcontroller/getDatainpage" class="up-btn up-btn-sm up-btn-default u-add"> <i class="icon-u-add-round"></i>新增
                    </a> --%>
			</div>
		</form>
	...
	<!-- 新增编辑信息分类表的第一层弹框-->
	<div id="layer-xxfl" class="u-padding-top15 up-container-fluid"
		style="display: none">
		<form class="u_form" action="">
			<div class="up-form-group up-col-sm-24">
				<label class="up-control-label">主题 </label> <select name=""
					id="xxzt" class="up-form-control">
			...
			<div class="up-form-group up-col-sm-24">
				<label class="up-control-label">信息分类名称 </label>
				<div class="up-col-sm-24">
					<input type="text" name="" id="xxmc" class="up-form-control"
						placeholder="">
				</div>
			</div>
			<div class="up-form-group up-col-sm-24">
				...
				<div class="up-col-sm-24">
					<input type="text" name="" id="xxbm" class="up-form-control"
						placeholder="">
				</div>
			</div>
			<div class="up-form-group up-col-sm-24">
				<label class="up-control-label">表描述 </label>
				<div class="up-col-sm-24">
					<input class="up-form-control" rows="1"  name="" id="xxms" style="resize:none" />
				</div>
			</div>
		</form>
	...
	<div id="layer-editor" class="u-padding-top15 up-container-fluid"
		style="display: none">
		<form class="u_form"  style="" id="gridForm">
			<table width="100%">
				<tbody>
					<tr>
    ...
    require(['uskin'], function() {
        ...
        $("#u_pagination").pagination({
            ...
            noInfoText: "没有任何数据",
            remote : {
                url : "${basePath}/xxflcontroller/getMessagedata",
                params: { zt: "" ,meg:""},
                success:function(data){
                	recordList=data.list;
                	var infoStr = '';	
                	for(var i=0;i<recordList.length;i++){
                		infoStr=infoStr+"<tr><td><div style='width:"+(widthWindow*0.2-16)+"px'  >"+recordList[i].fd_xxmc+"</div></td><td tdvalue='"+recordList[i].zid+"'><div style='width:"+(widthWindow*0.2-16)+"px'  >"+
                		recordList[i].fd_ztmc+"</div></td><td><div style='width:"+(widthWindow*0.2-16)+"px'  >"+recordList[i].fd_bm+"</div></td><td style=\"word-break:break-all;word-wrap:break-word\"><div style='width:"+(widthWindow*0.3-16)+"px;overflow:hidden;white-space: nowrap;text-overflow: ellipsis;' >"+
                		(recordList[i].fd_ms==null?"":recordList[i].fd_ms)+
                		"</div></td><td><div style='width:"+(widthWindow*0.1-16)+"px'  ><button class=\"up-btn up-btn-default up-btn-xs\" data-op=\""+recordList[i].id+"\" id=\"xxfl_"+recordList[i].id+"\"><i class=\"icon-u-edit\"></i>编辑</button></div></td></tr>";;
                	}
                	$("#tableBody").empty();
...
/*             		$("button[id^='xxfl_']").each(function(){	
            			...
            			$(document).on("click",opId,function(){
            				window.location.href="${basePath}/xxflcontroller/editmetadata?megid="+megid; 
            			});
            		});  */
                },
                totalName:'total',
        ...
        $(document).on("click","#selbtn",function() {
       	 var params={zt: $("#selid").val(),meg:encodeURI($("#optext").val())}
        	...
        	require(['layer'], function() {
        		layer.open({
                    ...
                    yes: function(index, layero) {
		                  	  if($("#xxzt").val()=="" ||  $("#xxbm").val()=="" || $("#xxmc").val()==""){
		              		    layer.alert("请输入主题、信息分类名称和表名！");
		              		    return;
		              	  }
		                	if(addlist.length==0){
		              		layer.alert("请填写字段信息!");
		              	...
		              	var param = {xxzt:$("#xxzt").val(),xxmc:$("#xxmc").val(),xxms:$("#xxms").val(),xxid:'xzxx',xxbm:$("#xxbm").val(),addlist:JSON.stringify(addlist)};
		                  	$.ajax({
		                  		type : "POST",
		                  		url : '${basePath}/xxflcontroller/addMegdata',
		                  		dataType : "text",
		                  		data: param,
		                  		success: function (data) {
		                              if(data=="ok"){                  
		                              layer.alert("操作成功!",{closeBtn:0},function(){
		                              	window.location.href="${basePath}/xxflcontroller/getmegDatapage"
		          						})
		                              	;  
		                               }else if(data=="repeat"){
		                              	 layer.alert("表名已存在!");
...
/*                              	$.ajax({
                            		type : "POST",
                            		url : '${basePath}/xxflcontroller/editmetadata',
                            		dataType : "json",
                            		data: {megid:megid},
                            		success: function (data) {
	                                	var obj=data.relist;
	                                	//$("#xxzt").find("option[text='"+obj.FD_ZTMC+"']").attr("selected","selected");
	                               	 	$("#xxzt").val(obj[0].ZID);
	                            	 	$("#xxbm").val(obj[0].FD_BM);
	                            	 	$("#xxms").val(obj[0].FD_MS);
	                            		$("#xxmc").val(obj[0].FD_XXMC); 
                            		   }
                            	    });	  */
                            ...
                            yes: function(index, layero) {       
                                	var param = {xxzt:$("#xxzt").val(),xxmc:$("#xxmc").val(),xxms:$("#xxms").val(),xxid:megid,xxbm:$("#xxbm").val(),editlist:JSON.stringify(editlist),addlist:JSON.stringify(addlist)};
                                    	$.ajax({
                                    		type : "POST",
                                    		url : '${basePath}/xxflcontroller/updateMegdata',
                                    		dataType : "text",
                                    		data: param,
                                    		success: function (data) {
                                                if(data=="ok"){
     ...
     function getJqgridofAdd(){
    	 require(["jqGrid"], function(grid) {
    		 $("#xxgrid").jqGrid({
                 url: "${basePath}/xxflcontroller/getxxmetadata",
                 datatype: "json",
                 postData:{xxid:"xzxx"},
                 mtype: "GET",
                 colModel: [{
 		                ...
 		                layer.open({
                             ...
                             yes: function(index, layero) {
					            ...
					            //判断字段与已存在字段重复
                             	$.ajax({
                             		type : "POST",
                             		url : '${basePath}/xxflcontroller/checkCols',
                             		dataType : "text",
