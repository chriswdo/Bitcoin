    ...
    <div class="up-container-fluid">
        <form class="up-form-inline u-margin-top20 u-margin-bottom20 up-clearfix">
               ...
               <div class="up-form-group">
                ...
                <div class="up-form-group">
				<button type="button"
					class="up-btn up-btn-sm up-btn-default up-btn-block" id="mxdyadd">
					<i class="icon-u-add-round"></i>新增
				</button>                
<%--                     <a href="${basePath}/mxdyController/getmxdyDatainpage" class="up-btn up-btn-sm up-btn-default u-add"> <i class="icon-u-add-round"></i>新增
</a> --%>
                </div>
        </form>
 	...
 	<!-- 新增编辑模型定义表的第一层弹框-->
	<div id="layer-mxdy" class="u-padding-top15 up-container-fluid"
		style="display: none">
		<form class="u_form" action="">
			<div class="up-form-group up-col-sm-24">
				<label class="up-control-label">类型 </label> <select name=""
					id="jclx" class="up-form-control">
					<option value="">请选择数据类型</option>
					<option value="1">数据集市</option>
					<option value="2">数据仓库</option>
				</select>
			...
			<div class="up-form-group up-col-sm-24">
				...
				<div class="up-col-sm-24">
					<input type="text"  class="up-form-control"  rows="1"  name="" id="xxms" style="resize:none" />
				</div>
			</div>
		</form>
    ...
    require(['uskin'], function() {
        ...
        $("#u_pagination").pagination({
            ...
            noInfoText: "没有任何数据",
            remote : {
                url : "${basePath}/mxdyController/getmxdyMessagedata",
                params: { zt: "" ,meg:""},
                success:function(data){
                	recordList=data.list;
                	var infoStr = '';	
                	for(var i=0;i<recordList.length;i++){
                		infoStr=infoStr+"<tr><td><div style='width:"+(widthWindow*0.25-16)+"px' >"+recordList[i].fd_jcmxmc+"</div></td><td><div style='width:"+(widthWindow*0.3-16)+"px;overflow:hidden;white-space: nowrap;text-overflow: ellipsis;' >"+
                		recordList[i].fd_ms+"</div></td><td><div style='width:"+(widthWindow*0.25-16)+"px' >"+recordList[i].fd_bm+
                		"</div></td><td tdvalue='"+recordList[i].fd_lx+"'><div style='width:"+(widthWindow*0.2-16)+"px' ><button class=\"up-btn up-btn-default up-btn-xs\" data-op=\""+recordList[i].id+"\" id=\"xxfl_"+recordList[i].id+"\">编辑</button></div></td></tr>";
                	}
                	$("#tableBody").empty();
...
/*             		$("button[id^='xxfl_']").each(function(){	
            			...
            			$(document).on("click",opId,function(){
            				window.location.href="${basePath}/mxdyController/editmxdymetadata?megid="+myid; 
            			});
            		});  */
                },
                totalName:'total',
        	...
        	require(['layer'], function() {
        		layer.open({
                    ...
                    success: function(layero, index){
                    	...
                    	$("#jclx").attr("disabled",false);  
                   	 
                      },
                    yes: function(index, layero) {
                    	if($("#xxmc").val()==""|| $("#xxbm").val()=="" ||$("#jclx").val()==""){
                    		layer.alert("请输入表名,名称,类型!");  
                    		return;
                    	}
                    	if(addlist.length==0){
                    		layer.alert("请填写字段信息!");
                    	...
                    	var param = {xxzt:$("#jclx").val(),xxmc:$("#xxmc").val(),xxms:$("#xxms").val(),xxid:'xzxx',xxbm:$("#xxbm").val(),addlist:JSON.stringify(addlist)};
                        	$.ajax({
                        		type : "POST",
                        		url : '${basePath}/mxdyController/addjcmxMegdata',
                        		dataType : "text",
                        		data: param,
                        		success: function (data) {
                                    if(data=="ok"){
...
/*                             	$.ajax({
                            		type : "POST",
                            		url : '${basePath}/mxdyController/editmxdymetadata',
                            		dataType : "json",
                            		data: {megid:megid},
                            		success: function (data) {
	                                	var obj=data.relist;
     ...
     function getJqgridofAdd(){
    	 require(["jqGrid"], function(grid) {
    		 $("#xxgrid").jqGrid({
                 url: "${basePath}/mxdyController/getjcmxmetadata",
                 datatype: "json",
                 postData:{xxid:"xzxx"},
                 mtype: "GET",
                 colModel: [{
...
/*                                  	$.ajax({
     ...
     $(document).on("change","#jclx",function() {
    	 // layer.alert(111);
        	var str=$("#jclx").val();
        	var infoStr='';
        	if(str=="2"){
        		$("#FD_ZDLX").empty();
        		infoStr=infoStr+"<option value=\"请选择类型\">请选择类型</option><option value=\"VARCHAR\">VARCHAR</option><option value=\"DATE\">DATE</option><option value=\"TIMESTAMP\">TIMESTAMP</option><option value=\"BIGINT\">BIGINT</option><option value=\"INTEGER\">INTEGER</option><option value=\"NUMERIC\">NUMERIC</option>";   
                $("#FD_ZDLX").append(infoStr); 
                }else{
            		$("#FD_ZDLX").empty();
            		infoStr=infoStr+"<option value=\"请选择类型\">请选择类型</option><option value=\"VARCHAR\">VARCHAR</option><option value=\"DATE\">DATE</option><option value=\"TIMESTAMP\">TIMESTAMP</option><option value=\"NUMBER\">NUMBER</option><option value=\"NUMERIC\">NUMERIC</option>";   
                    $("#FD_ZDLX").append(infoStr); 
                }      	
        });   
    	   ...
    	   $("#FD_ZDCD").parent().find("span").remove();
            if($("#FD_ZDLX").val()=="VARCHAR"){
         	   $("#FD_ZDJD").val("");
              	 $("#FD_ZDCD").removeAttr("disabled");
              	 $("#FD_ZDJD").attr("disabled","disabled");
               }else if($("#FD_ZDLX").val()=="DATE"||$("#FD_ZDLX").val()=="TIMESTAMP"||$("#FD_ZDLX").val()=="INTEGER"||$("#FD_ZDLX").val()=="BIGINT"){
            	   $("#FD_ZDJD").val("");
            	   $("#FD_ZDCD").val("");
            	   $("#FD_ZDCD").attr("disabled","disabled");
              	 $("#FD_ZDJD").attr("disabled","disabled");
