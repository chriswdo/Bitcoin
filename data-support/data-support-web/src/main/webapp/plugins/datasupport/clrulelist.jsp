		...
		<div id="layer-check" class="u-padding-top15 up-container-fluid"
			...
			<div class="up-form-group " style="">
				...
				<div class="up-row" style="max-width: 700px">
					<div class="up-col-md-24">
						<divg id="u_paginationadd" class="up-pull-right"></div>
					</div>
				</div>
			</div>
...
<!-- 						<input type="text" name="" id="rulename1" class="up-form-control"
    ...
    require(['uskin'], function() {
        ...
        $("#u_pagination").pagination({
            ...
            noInfoText: "没有任何数据",
            remote : {
                url : "${basePath}/rulemacontroller/ajaxRulelistdataBygzmc?lx=1",
                success:function(data){
...
/*             		$("button[id^='rule_']").each(function(){	
            			...
            			$(document).on("click",opId,function(){
            				window.location.href="${basePath}/rulemacontroller/cleditRuledata?jobname="+jobname; 
            			});
            		}); */
                },
                totalName:'total',
        ...
        $("#u_paginationadd").pagination({
            ...
            noInfoText: "没有任何数据",
            remote : {
                url : basePath+"/rulemacontroller/getclJoblist",
                success:function(data){
                	recordList=data.list;
                	var infoStr = '';	
                	for(var i=0;i<recordList.length;i++){
                		...
                		"\"  id=\"option_"+recordList[i].id_job+"\"><i class=\"icon-u-check\"></i>选择</button><button style=\"margin-bottom:0;margin-left:5px\"  class=\"up-btn up-btn-default up-btn-xs\"    id=\"detail_btn\"  onclick='getJobview(\""+recordList[i].id_job+"\")'><i class=\"icon-u-check\"></i>详情</button></td></tr>";
                	}
	...
	/* 	                                	$.ajax({
		                                ...
		                                yes: function(index, layero) {
		                                	...
		                                	$.ajax({
		                                		type : "POST",
		                                		url : '${basePath}/rulemacontroller/updateclRuledata',
		                                		dataType : "text",
		                                		data: param,
		                                		success: function (data) {
		                                            if(data=="ok"){
		                                                layer.alert("操作成功!",{closeBtn:0},function(){
		                                                	window.location.href="${basePath}/rulemacontroller/getclruleDatapage"
		                            						});
		                                             }else{
		                                            	 layer.alert("操作失败！");
		                                             }
		   ...
		   gzid="";	
            require(['layer'], function() {
                layer.open({
                    ...
                    success: function(layero, index){
       /*              	$("#dejob").attr('disabled',true);  
                    	$("#dejob").removeClass("up-btn-default"); */
                 ...
                 var param = {rulename:$("#rulename1").val(),sryid:$("#sr").val(),srbid:" ",scyid:$("#sc").val(),scbid:" ",jobid:$("#update_jobid").val(),jobname:$("#jobid").val(),jobdesc:$("#jobdesc").val()};
                	$.ajax({
                		type : "POST",
                		url : '${basePath}/rulemacontroller/saveclRuledata',
                		dataType : "text",
                		data: param,
                		success: function (data) {
                            if(data=="ok"){
