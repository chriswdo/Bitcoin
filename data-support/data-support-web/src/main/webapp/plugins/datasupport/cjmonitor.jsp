...
<body class="u_scroll">
    ...
    require(['uskin'], function() {
        ...
        $("#u_pagination").pagination({
            ...
            noInfoText: "没有任何数据",
            remote : {
                url : "${basePath}/monitorrule/getRulemonitordata?lx=0",
                success:function(data){
                    ...
                    for(var i=0;i<recordList.length;i++){
                        infoStr=infoStr+"<tr><td>"+recordList[i].FD_GZMC+"</td><td>"+
                                ...
                                "</td><td><button class=\"up-btn up-btn-default up-btn-xs\" data-op=\""+recordList[i].ID+"\" id=\"rule_"+recordList[i].ID+"\">运行日志</button><button class=\"up-btn up-btn-default up-btn-xs\" data-op=\""+recordList[i].ID+"\" id=\"ycju_"+recordList[i].ID+"\">异常记录</button></td></tr>";
                    }
                    $("tbody").empty();
                    ...
                    $("button[id^='rule_']").each(function(){
                        ...
                        $(document).on("click",opId,function(){
                            window.location.href="${basePath}/monitorrule/jumpPage?jobid="+jobid+"&flag=0";
                        });
                    });

                    $("button[id^='ycju_']").each(function(){
                        var jobid=$(this).attr("data-op");
                        var opId="#ycju_"+jobid;
                        //layer.alert(opId+"///"+jobname);
                        //console.log(opId);
                        $(document).on("click",opId,function(){
                            window.location.href="${basePath}/monitorrule/jumpPageycju?jobid="+jobid+"&flag=0";
                        });
                    });
                },
                totalName:'total',
