...
<body class="u_scroll">
    ...
    require(['uskin'], function() {
        ...
        $("#u_pagination").pagination({
            ...
            noInfoText: "没有任何数据",
            remote : {
                url : "${basePath}/monitorrule/getRulemonitordata?lx=1",
                success:function(data){
            		...
            		$("button[id^='rule_']").each(function(){	
            			...
            			$(document).on("click",opId,function(){
            				window.location.href="${basePath}/monitorrule/jumpPage?jobid="+jobid+"&flag=1"; 
            			});
            		});
                },
                totalName:'total',
...
/*         	$.post('${basePath}/monitorrule/getRulemonitordata', {gzmc:$("#optext").val()},
        			function(data) {
            	recordList=data.list;
            	//console.log(recordList);
            	var infoStr = '';	
        		...
        		$("button[id^='rule_']").each(function(){	
        			...
        			//console.log(opId);
        			$(document).on("click",opId,function(){
        				window.location.href="${basePath}/monitorrule/getRuledetail?jobid="+jobid; 
        			});
        		});
        	},'json'); */
        })      
